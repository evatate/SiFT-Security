...
class SiFT_MTP:
    ...
    def receive_msg(self):
        ...
        if parsed_msg_hdr['typ'] == self.type_login_req:
            # Login request has extra etk field
            body_len = msg_len - self.size_msg_hdr
            epd_len = body_len - self.size_msg_mac - self.size_etk
        else:
            body_len = msg_len - self.size_msg_hdr
        ...
        # For login_req, receive encrypted temporary key
        etk = None
        if parsed_msg_hdr['typ'] == self.type_login_req:
            try:
                etk = self.receive_bytes(self.size_etk)
            except SiFT_MTP_Error as e:
                raise SiFT_MTP_Error('Unable to receive encrypted temporary key --> ' + e.err_msg)
        ...
        if parsed_msg_hdr['typ'] == self.type_login_req:
            # Login request uses temporary key (which should have been set by server after decrypting etk)
            if self.temp_key is None:
                raise SiFT_MTP_Error('Temporary key not set for login request')
            key = self.temp_key
        else:
        ...
        # Return message type, payload, and etk (if login_req)
        if etk:
            return parsed_msg_hdr['typ'], msg_payload, etk
        else:
    ...
    def send_msg(self, msg_type, msg_payload, etk=None):
        ...
        if msg_type == self.type_login_req:
            # Login request uses temporary key
            if self.temp_key is None:
                raise SiFT_MTP_Error('Temporary key not set for login request')
            key = self.temp_key
